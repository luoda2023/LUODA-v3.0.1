...
class ChatModel with ChangeNotifier {
  ...
  Future<void> _sendMessage(ChatMessage message) async {
      ...
      _selfDestructTimers[record.id] = Timer(
        ...
        () async {
          ...
          if (current.isOutgoing) {
            final destroyed =
                await DirectChatRepository.instance.mutateOutgoing(
    ...
    Future<void> onDirectSessionReady({
      ...
      final resolvedPeerId = peerId?.trim().isNotEmpty == true
          ? peerId!.trim()
          : _currentKey.peerId;
    ...
    Future<void> _handleEnvelope(
      ...
      switch (envelope.type) {
        ...
        case 'replica_request':
          ...
          final cursor = _parseCursor(envelope.data['cursor']);
          final records =
              await DirectChatRepository.instance.afterCursor(cursor);
          for (final record in records) {
            _sendWire(
              key,
    ...
    Future<void> _sendVoiceChunks(
      ...
      if (bytes.isEmpty || bytes.length > DirectVoiceStorage.maxClipBytes)
        return;
      final digest = sha256.convert(bytes).toString();
    ...
    ChatMessage _toChatMessage(DirectChatRecord record, ChatUser user) {
      ...
      return ChatMessage(
        ...
        customProperties: <String, dynamic>{
          ...
          if (record.replyToId.isNotEmpty)
            'ldesk_reply_to_id': record.replyToId,
    ...
    Future<bool> editMessage(ChatMessage message, String newText) async {
      ...
      insertMessage(key, _toChatMessage(updated, me));
      _sendWire(key,
          DirectChatEnvelope.edit(messageId: id, newText: trimmed).encode());
      notifyListeners();
      return true;
    }

...
class _VoiceMessageRecorderButtonState
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      _timer = Timer.periodic(const Duration(seconds: 1), (_) {
        if (!mounted) return;
        if (_elapsed.elapsed >= const Duration(seconds: 60)) {
  ...
  Widget build(BuildContext context) {
    final seconds = _elapsed.elapsed.inSeconds.clamp(0, 60);
    return Tooltip(
      message: translate(
        _recording ? 'Stop and send voice message' : 'Record voice message',
      ),
      child: SizedBox(
        width: 40,
        height: 36,
          ...
          icon: Stack(
            ...
            children: <Widget>[
              Icon(
                ...
                size: 22,
                color: _recording ? const Color(0xFFFA5151) : null,
              ),
              if (_recording)
                Positioned(
                  right: -9,

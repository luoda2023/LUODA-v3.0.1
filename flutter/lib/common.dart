...
class MyTheme {
  ...
  static SwitchThemeData switchTheme({bool dark = false}) {
    return SwitchThemeData(
      ...
      trackColor: MaterialStateProperty.resolveWith((states) {
        if (states.contains(MaterialState.disabled)) {
          return dark ? const Color(0xFF2A303B) : const Color(0xFFE5EAF1);
        }
        if (states.contains(MaterialState.selected)) {
          return dark ? accentDark : primary;
        }
        return dark ? const Color(0xFF3A424F) : const Color(0xFFC7D0DD);
      }),
    );
  }

  static RadioThemeData radioTheme({bool dark = false}) {
    return RadioThemeData(
      splashRadius: (isDesktop || isWebDesktop) ? 0 : kRadialReactionRadius,
      fillColor: MaterialStateProperty.resolveWith((states) {
        if (states.contains(MaterialState.disabled)) {
          return dark ? const Color(0xFF697386) : const Color(0xFF9AA7B8);
        }
        return dark ? accentDark : primary;
      }),
    );
  ...
  static CheckboxThemeData checkboxTheme({bool dark = false}) {
    return CheckboxThemeData(
      ...
      fillColor: MaterialStateProperty.resolveWith((states) {
        if (states.contains(MaterialState.disabled)) {
          return dark ? const Color(0xFF434B58) : const Color(0xFFD7DEE8);
        }
        if (states.contains(MaterialState.selected)) {
          return dark ? accentDark : primary;
        }
      ...
      side: BorderSide(
        color: dark ? const Color(0xFF7D899B) : const Color(0xFF8795A8),
        width: 1.4,
      ),
    );
  }
...
const kCustomClientWindowSize = Size(470, 500);
      ...
      case WindowType.Main:
        ...
        if (isWindows || isLinux) {
          if (isCustomClient) {
            ...
            // 普通版主窗口首次启动使用 1280×720；已保存尺寸仍按原逻辑恢复。
            try {
  ...
  if (type != null && id != null) {
    switch (type) {
      case UriLinkType.remoteDesktop:
        Future.delayed(Duration.zero, () {
          luodaWinManager.newRemoteDesktop(
            id!,
            password: password,
      ...
      case UriLinkType.fileTransfer:
        Future.delayed(Duration.zero, () {
          luodaWinManager.newFileTransfer(
            id!,
            password: password,
            forceRelay: forceRelay,
          );
      ...
      case UriLinkType.portForward:
        Future.delayed(Duration.zero, () {
          luodaWinManager.newPortForward(
            id!,
            false,
            password: password,
            forceRelay: forceRelay,
...
connectMainDesktop(
  ...
  if (isFileTransfer) {
    ...
    await luodaWinManager.newViewCamera(
      id,
      password: password,
      isSharedPassword: isSharedPassword,
      connToken: connToken,
    ...
    await luodaWinManager.newPortForward(
      id,
      isRDP,
      password: password,
      isSharedPassword: isSharedPassword,
    ...
    await luodaWinManager.newTerminal(
      id,
      password: password,
      isSharedPassword: isSharedPassword,
      connToken: connToken,

...
UINT __stdcall CreateStartService(__in MSIHANDLE hInstall)
{
    ...
    if (MyCreateServiceW(svcName, szSvcDisplayName, svcBinary)) {
        ...
        // Start service asynchronously via shell to avoid blocking the installer.
        // StartServiceW (SCM API) blocks until the service reaches SERVICE_RUNNING
        // or fails, which can take 30+ seconds if the binary has missing DLLs or
        // slow initialization. The async shell approach never blocks the installer.
        wchar_t szStartCmd[800] = { 0 };
        hr = StringCchPrintfW(szStartCmd, 800, L"/c sc start %ls", svcName);
        if (SUCCEEDED(hr)) {
            HINSTANCE shellResult = ShellExecuteW(NULL, L"open", L"cmd.exe", szStartCmd, NULL, SW_HIDE);
            if ((INT_PTR)shellResult <= 32) {
                WcaLog(LOGMSG_STANDARD, "Failed to start service via shell: %ls, error: 0x%02X", svcName, GetLastError());
            } else {
                WcaLog(LOGMSG_STANDARD, "Service \"%ls\" start initiated via shell.", svcName);
            }
        }

        // Brief polling: if the service starts within 3 seconds, log success.
        // If not, the fallback below will handle it. The service is SERVICE_AUTO_START
        // so it will start on next boot regardless.

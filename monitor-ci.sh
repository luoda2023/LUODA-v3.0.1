...
if [ -z "$GH_TOKEN" ]; then
    ...
    echo ""
    echo "或者手动触发流水线："
    echo "1. 访问 https://github.com/luoda2023/LUODA-v3.0.1/actions"
    echo "2. 点击 '运行流水线' 按钮"
    echo "3. 选择 'build-exe.yml' 流水线"
    exit 1
fi
...
echo "流水线链接：https://github.com/luoda2023/LUODA-v3.0.1/actions/runs/$pipeline_id"
echo ""
...
while [ $attempt -lt $max_attempts ]; do
    ...
    case "$state" in
        "success")
            echo ""
            echo "✅ 构建成功!"
            echo "📦 产物链接：https://github.com/luoda2023/LUODA-v3.0.1/actions/runs/$pipeline_id/artifacts"
            exit 0
            ;;
        "failure")
            echo ""
            echo "❌ 构建失败!"
            echo "📄 查看日志：https://github.com/luoda2023/LUODA-v3.0.1/actions/runs/$pipeline_id/jobs"
            exit 1
            ;;
        "cancelled")
            echo ""
...
echo ""
echo "⏰ 监控超时 (30 分钟)"
echo "📄 查看最新状态：https://github.com/luoda2023/LUODA-v3.0.1/actions/runs/$pipeline_id"
exit 3

...
fn password_cache_path() -> Option<PathBuf> {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        // Use ProgramData (C:\ProgramData) on Windows — a shared directory
        // accessible by both the user UI and the Windows Service (SYSTEM).
        ...
        Some(dir.join("LUODA").join(PASSWORD_CACHE_FILE))
    }
...
fn load_password_from_cache() -> Option<String> {
    if let Some(path) = password_cache_path() {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        let saved = Config::get_option("temporary-password-cache");
        if !saved.is_empty() && saved.len() >= 4 {
            return Some(saved);
        }
    }
    None
...
fn save_password_to_cache(pwd: &str) {
    // Save to file (shared across processes)
    if let Some(path) = password_cache_path() {
        if let Some(parent) = path.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        let _ = std::fs::write(&path, pwd);
    }
    // Also save to Config as fallback
    Config::set_option("temporary-password-cache".to_owned(), pwd.to_owned());
}

lazy_static::lazy_static! {
    pub static ref TEMPORARY_PASSWORD:Arc<RwLock<String>> = Arc::new(RwLock::new(load_or_generate_password()));
}

fn load_or_generate_password() -> String {
    if let Some(pwd) = load_password_from_cache() {
        return pwd;
    }
    let pwd = get_auto_password();
    save_password_to_cache(&pwd);
    pwd
}
...
pub fn update_temporary_password() {
    let pwd = get_auto_password();
    *TEMPORARY_PASSWORD.write().unwrap() = pwd.clone();
    save_password_to_cache(&pwd);
}

// Set the temporary password directly (used when receiving it via IPC
// so the service uses the exact same password as the UI process).
pub fn set_temporary_password(pwd: String) {
    *TEMPORARY_PASSWORD.write().unwrap() = pwd.clone();
    save_password_to_cache(&pwd);
}
